...
if ($Missing.Count -gt 0) {
    Write-Error "[ERROR] Missing required keys in configuration file: $($Missing -join ', ')"
    exit 1
}
...
try {
    ...
    if (-not $InstanceKey) {
        throw "[ERROR] SQL Server registry instance key not found for instance '$SQLInstance'"
    }
    ...
    Write-Error "[WARN] Failed to update SQL Server TCP/IP ports: $_"
}
